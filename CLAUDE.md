...
- **Markdown / LaTeX 渲染**: 模型消息统一走 `frontend/src/components/MarkdownRenderer.tsx`，集中启用 GFM + `remark-math-extended` + `rehype-katex` 并加载 KaTeX CSS；支持 `$...$` / `$$...$$` 及模型常见的 `\(...\)` / `\[...\]`，同行双美元与方括号仍按 display 渲染，`$5 or $10` 等货币对保持普通文本。slash 分隔符由 micromark 在 Markdown 语法上下文中原生解析，禁止在解析前用全局替换破坏 inline/fenced code、链接或流式未闭合文本；新增消息型界面不得直接引入 `react-markdown`
